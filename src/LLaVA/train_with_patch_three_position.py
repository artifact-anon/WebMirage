...
class DebugTrainer(AdversarialTrainer):
    ...
    def training_step(self, model, inputs):
        ...
        for step in range(pgd_steps):
            ...
            if clean_images_for_pgd is not None:
                ...
                if "image_id" in inputs:
                    ...
                    patch_boxes = torch.tensor([[x, y, 295, 323] for x, y in patch_positions], device=clean_images_for_pgd.device)
                else:
                    print("No image_id found, using single patch at (1074, 50)")
                    B = clean_images_for_pgd.shape[0]
                    patch_boxes = torch.tensor([[1085, 50, 295, 323]] * B, device=clean_images_for_pgd.device)
        ...
        # compute the loss for _E
        with torch.no_grad():
            # 1) Forward for logits ONLY (keep images, ids, masks)
            fwd_inputs = {k: v for k, v in batch_inputs.items() if k != "labels"}
            out = model(**fwd_inputs)
            logits = out.logits.float()            # [B, T_log, V]
            labels = batch_inputs["labels"]        # [B, T_lab]

            # 2) Make sequence lengths match BEFORE shifting
            L = min(logits.size(1), labels.size(1))
            if L < 2:
                self._special_token_losses.append(float("nan"))
            else:
                logits = logits[:, :L, :]          # [B, L, V]
                labels = labels[:, :L]             # [B, L]

                # 3) HF causal LM shift: logits[:, t-1] predict labels[:, t]
                shift_logits = logits[:, :-1, :]   # [B, L-1, V]
                shift_labels = labels[:, 1:]       # [B, L-1]

                # 4) Resolve id for leading-space ' E' (▁E)
                ids = self.tokenizer(" E", add_special_tokens=False).input_ids
                target_id = ids[0] if ids else 764  # fallback to your known id

                # 5) Build mask of positions we care about
                valid = shift_labels.ne(IGNORE_INDEX)
                is_target = shift_labels.eq(target_id)
                m = valid & is_target               # [B, L-1]

                if m.any():
                    # Efficient NLL for a single class: logsumexp - selected logit
                    token_logit = shift_logits[..., target_id]        # [B, L-1]
                    lse = torch.logsumexp(shift_logits, dim=-1)       # [B, L-1]
                    nll = (lse - token_logit)                         # = -log p(target)

                    token_loss = nll[m].mean().item()
                    self._special_token_losses.append(token_loss)
                else:
                    self._special_token_losses.append(float("nan"))
...
class AdversarialInferenceCallback(TrainerCallback):
    def __init__(self, tokenizer, system_prompt, user_request, grounding_prompt, assistant_answer, device, base_img_tensor, processor, base_img_pil, special_token_losses):
        self.tokenizer = tokenizer
        ...
        self.special_token_losses = special_token_losses
        # # Compose pipeline for tensor input (skip ToTensor)
    ...
    def on_epoch_end(self, args, state, control, model=None, **kwargs):
        if state.epoch is None or state.epoch <= 1300:
            return
        ...
        if self.special_token_losses:
            avg_special_token_loss = sum(self.special_token_losses) / len(self.special_token_losses)
        else:
            avg_special_token_loss = float('nan')
        print(f"[EPOCH END] Average special token ('▁E') loss: {avg_special_token_loss}")
        self.special_token_losses.clear()
...
def train(attn_implementation=None):
    ...
    image_path_3 = data_args.image_path_3
    base_img_pil_3 = Image.open(image_path_3).convert("RGB")
    base_img_tensor_3 = torch.from_numpy(np.array(base_img_pil_3)).permute(2, 0, 1).float() / 255.0
    base_img_tensor_3 = base_img_tensor_3.to(training_args.device)
    ...
    grounding_prompt = conv_data["user"][1]
    assistant_answer = conv_data["assistant"]
    special_token_losses = trainer._special_token_losses
    inference_callback = AdversarialInferenceCallback(tokenizer, system_prompt, user_request, grounding_prompt, assistant_answer, training_args.device, base_img_tensor_1, data_args.image_processor, base_img_pil_1, special_token_losses)
    ...
    if training_args.save_adversarial_images:
        ...
        perturbed_1[:, 50:345, 1085:1408] += model.adversarial.delta.detach().to(base_img_tensor_1.device, dtype=base_img_tensor_1.dtype).squeeze(0)
        ...
        perturbed_2[:, 50:345, 730:1053] += model.adversarial.delta.detach().to(base_img_tensor_1.device, dtype=base_img_tensor_1.dtype).squeeze(0)
        ...
        perturbed_3 = base_img_tensor_3.clone()
        perturbed_3[:, 50:345, 375:698] += model.adversarial.delta.detach().to(base_img_tensor_1.device, dtype=base_img_tensor_1.dtype).squeeze(0)
        img_to_save = perturbed_3.clone().cpu().squeeze(0).clamp(0, 1)
        adv_img_pil_3 = to_pil_image(img_to_save)
        img_path_3 = os.path.join(training_args.adversarial_images_dir, 'adversarial_image_with_patch_3.png')
        adv_img_pil_3.save(img_path_3)
        print(f"[INFO] Adversarial image 3 saved to {img_path_3}")
        ...
        if not hasattr(training_args, 'local_rank') or training_args.local_rank == 0:
            ...
            patch_w, patch_h = 323, 295
            adversarial_card = adv_img_pil_3.crop((patch_x, patch_y, patch_x + patch_w, patch_y + patch_h))
            pertubate_path_3 = os.path.join(training_args.adversarial_images_dir, 'adversarial_patch_3.png')
            adversarial_card.save(pertubate_path_3)
            print(f"[INFO] Adversarial patch 3 saved to {pertubate_path_3}")

            patch_1 = Image.open(pertubate_path_1).convert('RGB')
            patch_2 = Image.open(pertubate_path_2).convert('RGB')
            patch_3 = Image.open(pertubate_path_3).convert('RGB')
                ...
                for i in range(num_samples):
                    ...
                    try:
                        ...
                        if idx == 0:
                            ...
                            sample_img.paste(patch_3, (375, 50))
