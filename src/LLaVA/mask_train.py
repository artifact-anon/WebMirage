...
class DataArguments:
    ...
    mask_path: str = field(default=None,
                           metadata={"help": "Path to the mask data."})
...
class AdversarialPerturbation(torch.nn.Module):
    def __init__(self, shape, device, processor, mask):
        """
        ...
        self.delta = torch.nn.Parameter(
            (torch.rand(shape, device=device) - 0.5) * 2 * 0.01  # Uniform in [-0.01, 0.01]
        )
        self.device = device
...
class DebugTrainer(AdversarialTrainer):
    ...
    def training_step(self, model, inputs):
        ...
        pgd_epsilon = 32/255 # PGD L-infinity constraint for delta
        ...
        for step in range(pgd_steps):
            ...
            if clean_images_for_pgd is not None:
                perturbed_images_for_loss = model.adversarial(clean_images_for_pgd)
            ...
            elif hasattr(model, 'adversarial') and self.accelerator.is_main_process:
                if clean_images_for_pgd is None:
                    print("  PGD: Skipped update (no clean_images_for_pgd available).")
                elif not hasattr(model.adversarial, 'delta'):
                    print("  PGD: Skipped update (model.adversarial.delta parameter missing).")
                elif model.adversarial.delta.grad is None:
                    print("  PGD: Skipped update (delta.grad is None after backward pass).")
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            ...
            # print(f"[DEBUG] Sample {i}: labels shape={labels.shape}, total={num_total}, masked={num_masked}, unmasked={num_unmasked}")
            # if num_unmasked == 0:
            #     print(f"[WARNING] All labels are masked for sample {i}!")
...
class AdversarialInferenceCallback(TrainerCallback):
    def __init__(self, tokenizer, system_prompt, user_request, grounding_prompt, assistant_answer, device, base_img_tensor, processor, mask_tensor, base_img_pil):
        self.tokenizer = tokenizer
    ...
    def on_epoch_end(self, args, state, control, model=None, **kwargs):
        if state.epoch is None or state.epoch <= 900:
            return
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        sample_image_path = data_args.image_path
        ...
        mask_np = np.load(data_args.mask_path)  # shape: (H, W)
        mask_tensor = torch.from_numpy(mask_np).float().to(training_args.device)
        if mask_tensor.dim() == 2:
            mask_tensor = mask_tensor.unsqueeze(0).repeat(3, 1, 1)  # (3, H, W)

        adversarial_module = AdversarialPerturbation(orig_img_shape, device=training_args.device, processor=data_args.image_processor, mask=mask_tensor)
    ...
    image_path = data_args.image_path
    base_img_pil = Image.open(image_path).convert("RGB")
    # base_img_tensor = process_images([base_img_pil], data_args.image_processor, model.config).squeeze(0).to(training_args.device)
    base_img_tensor = torch.from_numpy(np.array(base_img_pil)).permute(2, 0, 1).float() / 255.0
    base_img_tensor = base_img_tensor.to(training_args.device)
    with open(data_args.inference_data_path, "r", encoding="utf-8") as f:
        conv_data = json.load(f)
    system_prompt = conv_data["system"]
    user_request = conv_data["user"][0]
    ...
    if mask_tensor.dim() == 2:
        mask_tensor = mask_tensor.unsqueeze(0).repeat(3, 1, 1)  # (3, H, W)
    inference_callback = AdversarialInferenceCallback(tokenizer, system_prompt, user_request, grounding_prompt, assistant_answer, training_args.device, base_img_tensor, data_args.image_processor, mask_tensor, base_img_pil)
    trainer.add_callback(inference_callback)

    
    # print("[DEBUG] Inference exactly before training")
    # inference_before_training(model, tokenizer, system_prompt, user_request, grounding_prompt, assistant_answer, training_args.device, data_args.image_processor, base_img_pil)
    ...
    if training_args.save_adversarial_images:
        adv_img_tensor = (base_img_tensor + model.adversarial.delta.detach().to(base_img_tensor.device, dtype=base_img_tensor.dtype) * mask_tensor.to(base_img_tensor.device, dtype=base_img_tensor.dtype))
        img_to_save = adv_img_tensor.clone().cpu().squeeze(0).clamp(0, 1)
        adv_img_pil = to_pil_image(img_to_save)
        img_path = os.path.join(training_args.adversarial_images_dir, 'adversarial_image_with_mask.png')
        adv_img_pil.save(img_path)
        print(f"[INFO] Adversarial image saved to {img_path}")
        ...
        if not hasattr(training_args, 'local_rank') or training_args.local_rank == 0:
            ...
            adversarial_card = adv_img_pil.crop((patch_x, patch_y, patch_x + patch_w, patch_y + patch_h))
            ...
            # --- test A, 40 remaining samples add pertubate card ---
            test_samples_dir = os.path.join(training_args.adversarial_images_dir, 'test_samples_1_unseen')
            ...
            for i in range(40):
                ...
                sample_img.paste(card, (1075, 50))
                sample_img.save(sample_path)
                print(f"[INFO] Overwrote {sample_path} with adversarial patch.")

            # --- test B, 30 trainning samples with 2 augmentations ---
            pertubated_train_samples_dir = os.path.join(training_args.adversarial_images_dir, 'train_samples')
            for i in range(30):
                sample_name = f'sample_{i}.png'
                sample_path = os.path.join(pertubated_train_samples_dir, sample_name)
                if not os.path.exists(sample_path):
                    print(f"[WARNING] {sample_path} does not exist, skipping.")
                    continue
                sample_img = Image.open(sample_path).convert('RGB')
            ...
            # test_b_dir = os.path.join(training_args.adversarial_images_dir, 'train_samples_with_augmentations')
            # os.makedirs(test_b_dir, exist_ok=True)
            # for i in range(30):
            #     img_name = f"sample_{i}.png"
            #     img_path = os.path.join(pertubated_train_samples_dir, img_name)
            #     factory = TestCaseFactory(input_image_path=img_path, output_dir=test_b_dir)
            #     factory.apply_and_save()

            # # --- test C, randomly select 20 testing samples with augmentations ---
            # test_c_dir = os.path.join(training_args.adversarial_images_dir, 'test_samples_with_augmentations')
            # os.makedirs(test_c_dir, exist_ok=True)
            # random.seed(17)
            # selected_indices = random.sample(range(30, 60), 20)
            ...
            #         print(f"[WARNING] {sample_path} does not exist, skipping.")
            #         continue
            #     factory = TestCaseFactory(input_image_path=sample_path, output_dir=test_c_dir)
            #     factory.apply_and_save()
            
            # --- test D, 40 unseen samples add pertubate card ---
            test_samples_dir = os.path.join(training_args.adversarial_images_dir, 'test_samples_2_unseen')
            card = Image.open(card_path).convert('RGB')
            for i in range(40):
                sample_name = f'sample_{i}.png'
                sample_path = os.path.join(test_samples_dir, sample_name)
            ...
            test_samples_dir = os.path.join(training_args.adversarial_images_dir, 'test_samples_3_unseen')
            card = Image.open(card_path).convert('RGB')
            for i in range(40):
                sample_name = f'sample_{i}.png'
                sample_path = os.path.join(test_samples_dir, sample_name)
