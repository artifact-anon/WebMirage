...
class AdversarialTrainer(CPMTrainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:  # type: ignore
        ...
        pgd_steps = getattr(self.args, "pgd_steps", 3)
        pgd_alpha = getattr(self.args, "pgd_alpha", 1 / 255)
        ...
        for step in range(pgd_steps):
            ...
            inputs["pixel_values"] = perturbed_batch
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    patch_w: int = field(default=0, metadata={"help": "Width of patch."})
    patch_h: int = field(default=0, metadata={"help": "Height of patch."})
    patch_x: int = field(default=0, metadata={"help": "X-coordinate of patch."})
    patch_y: int = field(default=0, metadata={"help": "Y-coordinate of patch."})
...
class AdversarialPerturbation(nn.Module):
    ...
    @staticmethod
    def _get_sliced_grid(image_size, max_slice_nums, scale_resolution, never_split=False):
        w, h = image_size
    ...
    def _split_to_patches(self, img: torch.Tensor, grid):
        """Split image tensor into patches according to grid.
        
        Args:
            img: Tensor of shape [C, H, W]
            grid: List [grid_x, grid_y] specifying the grid dimensions
            
        Returns:
            List of patch tensors, flattened in row-major order
        """
        patches = []
        _, H, W = img.shape
        grid_x = int(W / grid[0])
    ...
    def _slice_image(self, img: torch.Tensor):
        ...
        if grid is None:
            # Don't need to slice, just resize to ensure divisibility by patch_size
            best_size = self._find_best_resize(original_size, self.scale_resolution, self.patch_size, allow_upscale=True)
            source = F.interpolate(
                img.unsqueeze(0), size=(best_size[1], best_size[0]), 
            ...
            source = F.interpolate(
                img.unsqueeze(0), size=(best_resize[1], best_resize[0]), 
                mode="bicubic", align_corners=False, antialias=True
            ).squeeze(0)

            # 2. Compute refined size for patch slicing that matches the processor
            refine_size = self._get_refine_size(
                original_size, grid, self.scale_resolution, self.patch_size, allow_upscale=True
            )
            refine_img = F.interpolate(
                img.unsqueeze(0), size=(refine_size[1], refine_size[0]), 
                mode="bicubic", align_corners=False, antialias=True
            ).squeeze(0)

            # 3. Split into patches in the same order as the official implementation
...
def train():
    ...
    if training_args.adv_training and hasattr(model, "adversarial"):
        try:
            ...
            patch_w, patch_h = training_args.patch_w, training_args.patch_h
            patch_x, patch_y = training_args.patch_x, training_args.patch_y
