...
def train(args):
    ...
    trainer.add_callback(InferenceCallback(
        ...
        device=device,
        base_img=base_img_tensor_2,
        image_processor=processor,
        base_img_pil=base_img_pil_2,
    ))
    ...
    #             perturbed = base_img_tensor.squeeze(0).clone()
    #             perturbed[:, 50:345, 1085:1408] += delta.squeeze(0)
    #             adv_img_tensor = perturbed.clone().cpu().squeeze(0).clamp(0, 1)
    ...
    #                         idx = i % 2
    #                         if idx == 0:
    #                             perturbed = sample_img_tensor.clone()
    #                             perturbed[:, 50:345, 730:1053] += delta.squeeze(0)
    #                             adv_img_tensor = perturbed.clone().cpu().squeeze(0).clamp(0, 1)
    #                             pil_img = to_pil_image(adv_img_tensor.squeeze(0).detach().cpu())
    #                         elif idx == 1:
    #                             perturbed = sample_img_tensor.clone()
